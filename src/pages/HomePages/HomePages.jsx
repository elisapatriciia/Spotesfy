            ...
            <div className="body">
                {
                    data.artist.albums?.filter(album=>state? album.name.toLowerCase().includes(state.toLowerCase()): true).map(album=>{
                        return(
                                <Card style={{ width: '18rem' }} className="card-container home-card grow" as={Link} to={`/detail/${album.id}/${ARTIS}`}>
                                    <Card.Img className="album-cov" variant="top" src={album.image} />
